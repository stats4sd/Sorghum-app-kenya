...
import {FullTrialResultsPage} from "./full-trial-results/full-trial-results"
...
export class PerformanceTabPage {
  ...
  constructor(private nav: NavController, params:NavParams, seedMasterData:SeedMasterData) {
    this.seedData=params.data;
    console.log(this.seedData);
    this.trialMeta=seedMasterData.trialMeta;
    this.seedMasterData=seedMasterData;
    this.calculateSeedTrialStats();
    visValues = this.generateVisValues();
    var vlSpec = getSpec();
    this.summaryFields=[
      {name:'Grain yield', units:'t/ha'},
      {name:'Plant Height', units:'cm'},
      {name:'Panicle Length', units:'cm'},
      {name:'Panicle Width', units:'cm'},
      {name:'Days to 50% Flowering', units:''}
    ];
    var embedSpec = {
      mode: "vega-lite",
      spec: vlSpec
    ...
    vg.embed("#summaryVis", embedSpec, function(error, result) {
      // Callback receiving the View instance and parsed Vega spec
      // result.view is the View, which resides under the '#vis' element
    });
  }
  calculateSeedTrialStats(){
    var temp=[];
    for (let trial in this.trialMeta){
      for (let item of  this.trialMeta[trial].trials){
        if(item.Genotypes==this.seedData.Genotypes){
          temp.push(item)
        }
      }
      //calculate stats and push json entries to array for use in template
      var stats=this.seedMasterData.calculateStats(temp);
      this.trialMeta[trial].seedStats=stats;
      this.trialMeta[trial].trialName=trial;
      this.trialMetaArray.push(this.trialMeta[trial]);
    }
    console.log(this.trialMetaArray)
  }
  generateVisValues(){
    var vals=[];
    for (let trial of this.trialMetaArray){
      for (let value of trial.seedStats['Grain yield'].values){
        vals.push({
          "grain yield":value,"variety":this.seedData.Genotypes,"year":trial.trialName,"site":'all sites'
        })
      }
    }
    console.log(vals);
  ...
  showFullResults(trial){
    let modal = Modal.create(FullTrialResultsPage, trial);
...
var values=[{"yield":27,"variety":"Manchuria","year":1931,"site":"University Farm"},
